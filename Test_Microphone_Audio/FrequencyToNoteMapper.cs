...
{
    public class FrequencyToNoteMapper
    {
        private static FrequencyToNoteMapper _instance;

        public static FrequencyToNoteMapper Instance => _instance ??= new FrequencyToNoteMapper();

        public static readonly Dictionary<string, double> FrequencyTable = new Dictionary<string, double>
    {
        ...
        public static string GetClosestNote(double frequency)
        {
            var closest = FrequencyTable.OrderBy(kv => Math.Abs(kv.Value - frequency)).First();
            return $"{closest.Key} ({closest.Value} Hz)";
        }
    }
}
